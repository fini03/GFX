...
import { toggleShadows } from './main.js';
...
const modes = {
    ...
    MODE_LIGHT: 3 // Changes apply only on the light
};
...
class KeyboardInteraction {
    ...
    registerEvents(camera, light, objects, shaders, globalTransformationMatrix) {
        this.#objects = objects;
        this.#shaders = shaders;
        this.#selectedShader = shaders.get("sBaseGD");
        document.addEventListener('keydown', (event) => this.#handleKeyDown(event, camera, light, objects, shaders, globalTransformationMatrix));
    }
    ...
    #selectObject(newObjectSelection) {
    ...
    #handleKeyDown(event, camera, light, objects, shaders, globalTransformationMatrix) {

        // Activate MODE_CAMERA
        if (event.code === 'Space') {
        ...
        if(isFinite(event.key)) {
            ...
            if (index === 0) {
                ...
                this.#index = index - 1;
                this.#selectObject(objects[this.#index]);
        ...
        switch(event.key) {
            ...
            case 'w':
                // Gouraud/diffuse
                this.#selectShader(shaders.get("sBaseGD"));
                toggleShadows(false);
                break;
            case 'e':
                // Gouraud/specular
                this.#selectShader(shaders.get("sBaseGS"));
                toggleShadows(false);
                break;
            case 'r':
                // Phong/diffuse
                this.#selectShader(shaders.get("sBasePD"));
                toggleShadows(false);
                break;
            case 't':
                // Phong/specular
                this.#selectShader(shaders.get("sBasePS"));
                toggleShadows(false);
                break;
            ...
            case 'h':
                ...
                const shadersArr = [
                    shaders.get("sBaseGD"),  shaders.get("sBaseGS"),  shaders.get("sBasePD"), shaders.get("sBasePS"),
                    shaders.get("sBaseGDS"), shaders.get("sBaseGSS"), shaders.get("sBasePDS"), shaders.get("sBasePSS")
                ];

                const shaderIndex = shadersArr.indexOf(shader);
                const toggleShadowsIndex = shaderIndex > 3 ? shaderIndex - 4 : shaderIndex + 4;

                this.#selectShader(shadersArr[toggleShadowsIndex]);
                toggleShadows(shaderIndex <= 3);
                break;
        }
        ...
        } else if (this.#mode === modes.MODE_LIGHT) {

            /* --------- Applying the changes on the light  --------- */

            switch(event.key) {
                // Rotations
                case 'i':
                    // Rotate cw about the xAxis
                    light.rotate('x', -ROTATION_ANGLE);
                    break;
                case 'k':
                ...
                case 'j':
                 ...
                 // Translations
                case 'ArrowUp':
                    // Moves light up -> positve yAxis
                    light.translate(0, KEYBOARD_SENSITIVITY, 0);
                    break;
                case 'ArrowDown':
                    // Moves light down -> negative yAxis
                    light.translate(0, -KEYBOARD_SENSITIVITY, 0);
                    break;
                case 'ArrowLeft':
                    // Moves light right -> negative xAxis
                    light.translate(-KEYBOARD_SENSITIVITY, 0, 0);
                    break;
                case 'ArrowRight':
                    // Moves light left -> positve xAxis
                    light.translate(KEYBOARD_SENSITIVITY, 0, 0);
                    break;
                case ',':
                    // Moves light forwards
                    light.translate(0, 0, KEYBOARD_SENSITIVITY);
                    break;
                case '.':
                    // Moves light backwards
                    light.translate(0, 0, -KEYBOARD_SENSITIVITY);
                    break;
            }
